DROP TABLE IF EXISTS PASSAGE_PRISON;
DROP TABLE IF EXISTS PASSAGE_PARK;
...
idU INTEGER,
idT INTEGER,
dateT DATE DEFAULT CURRENT_TIMESTAMP,
...
date DATE NOT NULL,
CONSTRAINT Fki1_UP PRIMARY KEY (idP)
);
...
CONSTRAINT Fki1_UP FOREIGN KEY (idP) REFERENCES PION(idP)
);
...
nomCase Varchar(20),
CONSTRAINT Pk_C PRIMARY KEY (idC)
);
...
CREATE TABLE PASSAGE_PRISON (
idu INTEGER,
date_passage DATE DEFAULT CURRENT_TIMESTAMP,
CONSTRAINT  Fk_pp FOREIGN KEY (idU) REFERENCES UTILISATEUR(idU)
);

CREATE TABLE PASSAGE_PARK (
idu INTEGER,
date_passage DATE DEFAULT CURRENT_TIMESTAMP,
CONSTRAINT  Fk_ppark FOREIGN KEY (idU) REFERENCES UTILISATEUR(idU)
);
